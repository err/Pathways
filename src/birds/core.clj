...
(defn stop-audio-loop [] (reset! *q-alive?* false))
  ...
  ([opt]
     (let [now @*time*
	   ...
	   songs    (or songs (into (sorted-map)
				    (map #(song-entry % {:tutor id})
					 (random-song-bank (or bank-size @*SONG-BANK-SIZE*)))))]
       {:id  id
	:pos pos
	:vel vel
	:frc frc
			...
			(if (not (:singing? new-b))
			  (do
			    (when (not @*mute?*)
			      ...
			      (send *player* (fn [s args] (tweet args)) (first (:notes response)))
		...
		(do
		  (if (and (not (student? b)) (not (:singing? b)) ;can't already be singing
		    ...
		    (let [song-type  (rand-nth (keys (:songs new-b)))
		      ...
		      (when (not @*mute?*)
			(send *player* (fn [s args] (tweet args)) song-notes)
